      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-14 flex flex-col gap-4 border-t border-grey-light pt-6 text-[0.75rem] text-grey md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-6">
                ...
                <a href={`mailto:${SITE_CONFIG.email}`} className="text-grey hover:text-ink">{SITE_CONFIG.email}</a>
                <a href={`tel:${SITE_CONFIG.phone}`} className="text-grey hover:text-ink">{SITE_CONFIG.phone}</a>
              </div>

              <div className="flex items-center gap-4">
                <nav className="flex gap-3">
                  <SocialLink href={SITE_CONFIG.social.facebook} label="Follow us on Facebook">
                  ...
                  <SocialLink href={SITE_CONFIG.social.instagram} label="Follow us on Instagram">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden>
                      <rect x="3" y="3" width="18" height="18" rx="5" />
                      <path d="M16 11.37A4 4 0 1 1 12.63 8" />
                    </svg>
                  </SocialLink>
                  <SocialLink href={SITE_CONFIG.social.twitter} label="Follow us on X">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                  ...
                  <SocialLink href={SITE_CONFIG.social.linkedin} label="Follow us on LinkedIn">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                  ...
                  <SocialLink href={SITE_CONFIG.social.whatsapp} label="Chat with us on WhatsApp">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
...
function SocialLink({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  if (!href) return <span aria-hidden className="w-11 h-11 inline-flex items-center justify-center" />;
  return (
    <a
      href={href}
      target="_blank"
