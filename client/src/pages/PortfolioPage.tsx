...
function PortfolioCard({ item }: { item: PortfolioItem }) {
  return (
    <article className="studio-plane group relative overflow-hidden border border-paper-deep bg-paper-white text-text-primary">
      <img
        src={item.image}
        alt={item.title}
        className="h-[560px] w-full object-cover"
      />
      <div className="absolute inset-0 bg-ink-rich/0 transition-colors duration-500 group-hover:bg-ink-rich/20" />
      <div className="absolute bottom-0 left-0 right-0 border-t border-paper-deep bg-paper/95 p-8">
        <p className="text-[0.75rem] uppercase tracking-[0.3em] text-brass">
          {item.category} - {item.year}
        </p>
        <h2 className="mt-4 text-2xl font-semibold uppercase tracking-[-0.03em] text-text-primary">
          {item.title}
        </h2>
