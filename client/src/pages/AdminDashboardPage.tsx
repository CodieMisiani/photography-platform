        ...
        <div className="mt-10 grid border-t border-grey-light md:grid-cols-2">
          {dashboardLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="border-b border-grey-light py-6 text-[0.8rem] font-semibold uppercase tracking-[0.25em] text-grey transition-colors hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink md:odd:border-r md:odd:pr-6 md:even:pl-6"
            >
              {link.label}
            </Link>
          ))}
